...
import { ExternalLink, Github, ArrowRight, X, AlertCircle, CheckCircle2, Cpu, ShoppingBag, Globe, FileText } from "lucide-react";
...
export const ProjectCard: React.FC<ProjectCardProps> = ({ project }) => {
  ...
  const getCategoryIcon = () => {
    if (project.technologies.some((t) => t.toLowerCase().includes("shopify"))) {
      return <ShoppingBag className="w-8 h-8 text-[#10B981]" />;
    }
    if (project.technologies.some((t) => t.toLowerCase().includes("wordpress"))) {
          ...
          <div className="relative h-48 sm:h-56 w-full overflow-hidden bg-[#08090C]">
            {project.imageUrl ? (
              ...
              <div className="absolute inset-0 bg-gradient-to-br from-[#0F131C] via-[#161B26] to-[#0A0D14] flex flex-col items-center justify-center p-6 text-center border-b border-slate-800/80">
                <div className="p-3 rounded-2xl bg-[#08090C]/80 border border-slate-800 mb-2 shadow-inner">
                  {getCategoryIcon()}
                </div>
                <h4 className="text-sm font-bold text-white max-w-[200px] line-clamp-1 mb-1.5">
                  {project.title}
                </h4>
                <span className="px-2.5 py-1 text-[10px] font-mono rounded-full bg-slate-900/90 text-amber-300/90 border border-amber-500/20 backdrop-blur-md">
        ...
        <div className="px-6 pb-6 pt-0 flex items-center justify-between border-t border-slate-800/60 mt-auto">
          ...
          <div className="flex items-center gap-3 pt-4">
            ...
            {project.demoUrl ? (
              ...
              <button
                onClick={() => setIsModalOpen(true)}
                className="px-2.5 py-1 text-[11px] font-mono rounded-lg bg-[#161B26] text-[#10B981] border border-[#10B981]/30 hover:bg-[#10B981]/15 transition-colors"
              >
                Case Study
              </button>
            )}
            ...
            >
              ...
              <div className="flex flex-wrap gap-3 pt-4 border-t border-slate-800">
                {project.demoUrl ? (
                  ...
                  <span className="px-4 py-2 rounded-xl bg-[#161B26] text-slate-400 text-xs font-mono border border-slate-700">
                    Live Store URL Unavailable
                  </span>
                )}
                {project.githubUrl && (
                  <a
